...
# Use this code block to drop the problematic tables:
spark.sql("DROP TABLE IF EXISTS VFADataValidation")
spark.sql("DROP TABLE IF EXISTS ProjectSummaryData")
print("Dropped existing VFADataValidation and ProjectSummaryData tables (if they existed).")
...
spark = SparkSession.builder.appName("VFA_RAP_Refactoring_Final_Fixed_V6").getOrCreate()

# --- 0. Load All Lookup and Source Tables from Lakehouse ---
try:
    configuration_df = spark.table("ConfigurationTable")
    iosf_df = spark.table("IoSF")
    ioaf_df = spark.table("IoAF")
    impactoffailure_df = spark.table("ImpactOfFailure")
    conditionofsystem_df = spark.table("ConditionOfSystem")
    planforasset_df = spark.table("PlanForAsset")
    reasonforwork_df = spark.table("ReasonForWork")
    vfa_ssrs_raw_df = spark.table("VFA_SSRS")
    rap_ssrs_df = spark.table("RAP_SSRS").select(
    ...
    annual_planner_b2025_df = spark.table("AnnualPlanner_B2025")
    annual_planner_b2026_df = spark.table("AnnualPlanner_B2026")
...
df_2025 = annual_planner_b2025_df.select(
    F.col("Asset Number").alias("AssetID").cast("string"), 
    F.col("REQ ID").alias("REQID").cast("string")
...
print(vfa_ssrs_staged_df.select("REQID").dtypes)
print(annual_planner_combined_df.select("REQID").dtypes)
...
def score_and_validate_requirements(source_df, annual_planner_combined_df):
    df = source_df
    ...
    df = df.join(annual_planner_combined_df, ["AssetID", "REQID"], "left_outer")
...
target_rapid = "B0182A-050" 

# Select the raw Error Level columns (1=Error, 2=Warn, None=Pass)
# We filter to show ONLY rows that have at least one issue (Not Null)
debug_df = vfa_data_validation_df.filter(F.col("RAPID") == target_rapid).select(
    "REQID", 
    "Uniformat",
    "Category",
    "VFASystemGroup",
    ...
    "ReqDescErrorLevel",
    "ReqNameErrorLevel",
    "UniformatErrorLevel",   # Is this 1?
    "CategoryErrorLevel",    # Is this 1?
    "PriorityErrorLevel",    # Is this 1?
    "ImpactErrorLevel",      # Is this 1?
    "SystemGroupErrorLevel",  # Is this 1?
    "PlannedStatus"
)

print(f"--- Breakdown for Project {target_rapid} ---")
display(debug_df)

# Calculate what the Sum SHOULD be manually
print("--- Calculated Totals ---")
debug_df.select(
    (
        F.sum(
            F.when(F.col("UniformatErrorLevel") == 1, 1).otherwise(0) +
            F.when(F.col("CategoryErrorLevel") == 1, 1).otherwise(0) +
            F.when(F.col("PriorityErrorLevel") == 1, 1).otherwise(0) +
            F.when(F.col("ImpactErrorLevel") == 1, 1).otherwise(0)
        ).alias("Expected_Scoring_Errors")
    ),
    (
        F.sum(
            F.when(F.col("SystemGroupErrorLevel") == 1, 1).otherwise(0)
        ).alias("Expected_Maintenance_Errors")
    )
).show()
...
# --- 1. SETUP ---
new_df = spark.table("ProjectSummaryData")
old_df = spark.table("pmbcp_projectsummarydata_prod") # <--- REPLACE THIS WITH ACTUAL TABLE NAME
...
# --- 1. SETUP ---
new_df = spark.table("VFADataValidation")
old_df = spark.table("pmbcp_vfadatavalidation_prod") # <--- REPLACE THIS
...
# --- DEBUG: FIND THE PHANTOM SCORING ERRORS ---
target_rapid = "B0086A-090"

print(f"--- Deep Dive for Project: {target_rapid} ---")

# We look at the 4 columns that make up 'VFA_Scoring_Error_s'
# If you see a '1' in any of these, that is your culprit.
display(vfa_data_validation_df
    .filter(F.col("RAPID") == target_rapid)
    .select(
        "REQID",
        "Uniformat",
        "MasterSystemGroup", # If this is null, you have a Uniformat Error
        F.col("UniformatErrorLevel").alias("Uniformat_Err"),
        F.col("CategoryErrorLevel").alias("Category_Err"),
        F.col("PriorityErrorLevel").alias("Priority_Err"),
        F.col("ImpactErrorLevel").alias("Impact_Err")
    )
)
...
# Run this to see what text we are actually working with
print(f"Original Config Message: '{MSG_ERROR_REQSYSGRP_MISSING}'")
print(f"Does it contain the placeholder? {'{MasterSystemGroup}' in MSG_ERROR_REQSYSGRP_MISSING}")
...
# --- RAP ERROR VALIDATION TOOL ---
target_rapid = "B0086A-090" # <--- REPLACE THIS with your Project ID

print(f"==================================================================")
print(f"   FORENSIC ANALYSIS FOR PROJECT: {target_rapid}")
print(f"==================================================================")

# --- 1. REQUIREMENT LEVEL (The Raw Data) ---
# We select the Error Levels (1=Error, 2=Warning) for the columns that feed the summary.
print("\n--- 1. REQUIREMENT LEVEL BREAKDOWN (Rows) ---")
print("Look for '1's in these columns. '2's (Warnings) should be ignored.")

req_breakdown = vfa_data_validation_df.filter(F.col("RAPID") == target_rapid).select(
    "REQID",
    # Scoping Errors (Feeds VFA_Scoping_Error_s)
    F.col("ReqNameErrorLevel").alias("ReqName_Lvl"),
    F.col("ReqDescErrorLevel").alias("ReqDesc_Lvl"),
    
    # Scoring Errors (Feeds VFA_Scoring_Error_s)
    F.col("CategoryErrorLevel").alias("Category_Lvl"),
    F.col("PriorityErrorLevel").alias("Priority_Lvl"),
    F.col("ImpactErrorLevel").alias("Impact_Lvl"),
    F.col("UniformatErrorLevel").alias("Uniformat_Lvl"),
    
    # Maintenance Errors (Feeds VFA_Maintenance_Planning_Error_s)
    F.col("SystemGroupErrorLevel").alias("SysGrp_Lvl"),
    
    # Costing Errors (Feeds VFA_Costing_Error_s)
    F.col("EstCostErrorLevel").alias("Cost_Lvl")
)

display(req_breakdown)

# --- 2. EXPECTED CALCULATIONS (The Math Check) ---
# We manually calculate what the sum SHOULD be using Python logic (Count 1s only)
print("\n--- 2. MATH CHECK (Manual Calculation vs. Spark Calculation) ---")

# Helper to simulate our count_err logic
def sim_count(c): return F.when(F.col(c) == 1, 1).otherwise(0)

math_check = req_breakdown.groupBy().agg(
    # Manually calc Scoping
    F.sum(sim_count("ReqName_Lvl") + sim_count("ReqDesc_Lvl")).alias("CALC_Scoping_Errors"),
    # Manually calc Scoring
    F.sum(sim_count("Category_Lvl") + sim_count("Priority_Lvl") + sim_count("Impact_Lvl") + sim_count("Uniformat_Lvl")).alias("CALC_Scoring_Errors"),
    # Manually calc Maintenance
    F.sum(sim_count("SysGrp_Lvl")).alias("CALC_Maint_Errors"),
    # Manually calc Costing
    F.sum(sim_count("Cost_Lvl")).alias("CALC_Cost_Errors")
)

# Get the actual values from the final table
actual_values = project_summary_data_df.filter(F.col("RAPID") == target_rapid).select(
    F.col("VFA_Scoping_Error_s").alias("ACTUAL_Scoping_Errors"),
    F.col("VFA_Scoring_Error_s").alias("ACTUAL_Scoring_Errors"),
    F.col("VFA_Maintenance_Planning_Error_s").alias("ACTUAL_Maint_Errors"),
    F.col("VFA_Costing_Error_s").alias("ACTUAL_Cost_Errors")
)

# Show them side-by-side
print("If these columns match, the logic is perfect.")
display(math_check.crossJoin(actual_values))

# --- 3. RAP LEVEL CHECKS (Project Fields) ---
print("\n--- 3. RAP LEVEL FIELD CHECKS (Pass-Throughs) ---")
print("Verifying that RAP Title/Desc checks are present.")
display(project_summary_data_df.filter(F.col("RAPID") == target_rapid).select(
    "RAPID", 
    "RAPTitleLengthCheck", 
    "RAPDescCheck", 
    "RAPConseqCheck", 
...
print(f"--- 1. DETAILED ROW ANALYSIS for {target_rapid} ---")
# We select the Text Message next to its Calculated Level
display(vfa_data_validation_df
    .filter(F.col("RAPID") == target_rapid)
    .select(
        "REQID", 
        # COST
        F.col("EstCostCheck").alias("Cost_Msg"), 
        F.col("EstCostErrorLevel").alias("Cost_Lvl"),
        
        # UNIFORMAT
        F.col("UniformatCheck").alias("Uni_Msg"), 
        F.col("UniformatErrorLevel").alias("Uni_Lvl"),
        
        # SYSTEM GROUP
        F.col("SystemGroupCheck").alias("Sys_Msg"), 
        F.col("SystemGroupErrorLevel").alias("Sys_Lvl"),
        
        # CATEGORY
        F.col("CategoryCheck").alias("Cat_Msg"), 
        F.col("CategoryErrorLevel").alias("Cat_Lvl"),
        
        # PRIORITY
        F.col("PriorityCheck").alias("Prio_Msg"), 
        F.col("PriorityErrorLevel").alias("Prio_Lvl")
    )
)

print(f"--- 2. FINAL COUNTS (What the App Sees) ---")
display(vfa_project_staging_df
    .filter(F.col("RAPID") == target_rapid)
    .select(
        "RAPID", 
        "VFA_Scoping_Error_s",
        "VFA_Costing_Error_s", 
        "VFA_Scoring_Error_s", 
        "VFA_Maintenance_Planning_Error_s"
    )
)
